  ...
  return (
    <div style={{position:'fixed',inset:0,background:'rgba(8,14,28,0.75)',backdropFilter:'blur(12px)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:1000,padding:20}} onClick={onClose}>
      <div style={{background:'var(--bg)',borderRadius:20,padding:'32px 36px',width:'100%',maxWidth:560,maxHeight:'90vh',overflowY:'auto',border:'1px solid var(--border)'}} onClick={e=>e.stopPropagation()}>
        <div style={{fontFamily:'Cormorant Garamond,serif',fontSize:26,fontWeight:300,color:'var(--navy)',marginBottom:24}}>{title}</div>
        {children}
      </div>
    </div>
  )
  ...
  return (
    <Modal title="Editar viaje" onClose={onClose}>
      ...
      <div style={{display:'flex',justifyContent:'flex-end',gap:10,marginTop:24}}>
        ...
        <button onClick={()=>f.name&&onSave({...f,budget:parseFloat(String(f.budget))||0})} style={{padding:'10px 20px',background:'#b87333',color:'white',border:'none',borderRadius:10,fontSize:13,fontWeight:600,cursor:'pointer',fontFamily:'DM Sans,sans-serif',boxShadow:'0 4px 16px rgba(184,115,51,0.3)'}}>Guardar cambios</button>
      </div>
    </Modal>
  )
}
  ...
  return (
    <div style={{display:'flex',height:'100vh',overflow:'hidden',background:'var(--bg)'}}>
      ...
      <div style={{flex:1,overflowY:'auto',background:'var(--bg)',display:'flex',flexDirection:'column'}}>
        <div className="mobile-header no-print" style={{display:'none',alignItems:'center',justifyContent:'space-between',padding:'14px 18px',background:'var(--bg-sidebar)',borderBottom:'1px solid rgba(255,255,255,0.05)',position:'sticky',top:0,zIndex:200}}>
        ...
        {trip && (
          <div className="no-print" style={{background:'var(--bg-card)',borderBottom:'1px solid var(--border)',position:'sticky',top:0,zIndex:100,padding:'20px 44px 0'}}>
            <div style={{display:'flex',alignItems:'center',gap:8,marginBottom:14,fontSize:12,color:'var(--text-light)'}}>
              <span onClick={()=>router.push('/')} style={{cursor:'pointer',color:'#b87333',fontWeight:500}} onMouseEnter={e=>e.currentTarget.style.opacity='0.7'} onMouseLeave={e=>e.currentTarget.style.opacity='1'}>Inicio</span>
            ...
            <div style={{display:'flex',alignItems:'flex-start',justifyContent:'space-between',marginBottom:18,flexWrap:'wrap',gap:10}}>
              ...
              <div style={{display:'flex',gap:8,alignItems:'center',flexWrap:'wrap'}}>
                <button onClick={()=>setShowEdit(true)} style={{padding:'9px 16px',background:'transparent',border:'1px solid var(--border)',borderRadius:10,fontSize:12,fontWeight:600,cursor:'pointer',fontFamily:'DM Sans,sans-serif',color:'var(--text-mid)',display:'flex',alignItems:'center',gap:6}}>✏️ Editar</button>
                ...
                <button onClick={handleDelete} style={{padding:'9px 12px',background:'transparent',border:'1px solid rgba(196,92,92,0.35)',borderRadius:10,fontSize:12,fontWeight:600,cursor:'pointer',fontFamily:'DM Sans,sans-serif',color:'#c45c5c'}}>🗑</button>
