...
function LazyMap({ lat, lng, active, onActivate }: { lat:number, lng:number, active:boolean, onActivate:()=>void }) {
  ...
  return (
    <div ref={ref} style={{position:'relative',height:130,overflow:'hidden',cursor:'pointer'}} onClick={onActivate}>
      {visible ? <iframe src={src} style={{width:'100%',height:'100%',border:'none',pointerEvents:active?'auto':'none'}} loading="lazy" />
        : <div style={{width:'100%',height:'100%',background:'var(--bg-cream-dark)'}}><div className="skeleton" style={{position:'absolute',inset:0,borderRadius:0}} /></div>}
      {!active&&visible&&<div style={{position:'absolute',inset:0,display:'flex',alignItems:'center',justifyContent:'center',background:'rgba(0,0,0,0.12)'}}><span style={{background:'white',borderRadius:20,padding:'5px 12px',fontSize:12,fontWeight:600,color:'#1a2744'}}>🗺️ Ver mapa</span></div>}
    </div>
  )
}
    ...
    <div className="tab-content-responsive fade-in" style={{padding:'32px 44px 64px'}}>
      ...
      {loading ? <>{[1,2,3,4].map(i=><Skel key={i} h={i===1?180:90} />)}</> : (
        <>
          ...
          {tab==='flights'   && <TabFlights    trip={trip} items={items} add={add} upd={upd} del={del} isReader={isReader} />}
          {tab==='itinerary' && <TabItinerary  trip={trip} items={items} add={add} upd={upd} del={del} isReader={isReader} />}
          {tab==='expenses'  && <TabExpenses   trip={trip} items={items} add={add} upd={upd} del={del} isReader={isReader} />}
          {tab==='places'    && <TabPlaces     trip={trip} items={items} add={add} upd={upd} del={del} isReader={isReader} />}
          {tab==='documents' && <TabDocuments  trip={trip} items={items} add={add} upd={upd} del={del} isReader={isReader} />}
...
function TabOverview({ trip, items }: any) {
  ...
  return (
    <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:18}}>
      ...
      {[{label:'Vuelos',num:fl,total:null,color:'#4a7fa5',sub:'registrados'},{label:'Actividades',num:done,total:acts,color:'#4a7fa5',sub:'realizadas'},{label:'Lugares',num:vis,total:items?.places?.length||0,color:'#4a7c59',sub:'visitados'},{label:'Equipaje',num:ck,total:ckT,color:'#b87333',sub:'ítems listos'}]
        .map((it,i)=>(
          <div key={i} style={{background:'var(--bg-card)',borderRadius:16,padding:'22px 24px',border:'1px solid var(--border)',boxShadow:'var(--shadow-card)'}}>
            ...
            {it.total!==null&&it.total>0&&<div style={{background:'var(--bg-cream-dark)',borderRadius:3,height:4,overflow:'hidden',marginTop:14}}><div style={{width:`${Math.round(it.num/it.total*100)}%`,height:'100%',borderRadius:3,background:it.color,transition:'width 1s cubic-bezier(0.16,1,0.3,1)'}} /></div>}
          </div>
        ))}
    </div>
  )
...
function TabFlights({ trip, items, add, upd, del, isReader }: any) {
    ...
    <div style={{background:'var(--bg-card)',borderRadius:16,border:'1px solid var(--border)',padding:'20px 24px',marginBottom:12,boxShadow:'var(--shadow-card)'}}>
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:16}}>
        <div style={{display:'flex',alignItems:'center',gap:10,flexWrap:'wrap'}}>
          <span style={{padding:'5px 12px',borderRadius:20,fontSize:11,fontWeight:700,background:f.type==='ida'?'rgba(74,127,165,0.1)':'rgba(74,124,89,0.1)',color:f.type==='ida'?'#4a7fa5':'#4a7c59'}}>{f.type==='ida'?'✈️ Ida':'🔄 Regreso'}</span>
        ...
        {!isReader&&<div style={{display:'flex',gap:6}}><IBtn onClick={()=>{setEditing(f);setModal(true)}}>✏️</IBtn><IBtn onClick={()=>del('flights',f.id)} color="#c45c5c">🗑</IBtn></div>}
      </div>
      ...
      {f.has_layover===1&&(<>
        ...
        <div style={{display:'flex',alignItems:'center',gap:12,flexWrap:'wrap'}}>
          <div style={{flex:1,minWidth:100}}><div style={{fontFamily:'Cormorant Garamond,serif',fontSize:22,fontWeight:300,color:'var(--navy)'}}>{f.layover_airport||'—'}</div><div style={{fontSize:12,color:'var(--text-mid)',marginTop:2}}>{f.layover_city}</div><div style={{fontSize:13,fontWeight:600,color:'var(--text)',marginTop:4}}>{f.layover_departure_time}</div></div>
          ...
          <div style={{flex:1,textAlign:'right',minWidth:100}}><div style={{fontFamily:'Cormorant Garamond,serif',fontSize:22,fontWeight:300,color:'var(--navy)'}}>{f.destination_airport||'—'}</div><div style={{fontSize:12,color:'var(--text-mid)',marginTop:2}}>{f.destination_city}</div><div style={{fontSize:13,fontWeight:600,color:'var(--text)',marginTop:4}}>{f.arrival_date} · {f.arrival_time}</div></div>
        </div>
      </>)}
  ...
  return (
    <div className="fade-in">
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:24}}>
        ...
        {!isReader&&<Btn onClick={()=>{setEditing(null);setModal(true)}} primary>＋ Agregar vuelo</Btn>}
      </div>
      {flights.length===0&&<Empty icon="✈️" title="Sin vuelos registrados" sub={isReader?'El organizador aún no ha agregado vuelos':'Agrega los detalles de tu vuelo de ida y regreso'} />}
      ...
      {modal&&<Modal title={editing?'Editar vuelo':'Nuevo vuelo'} onClose={()=>{setModal(false);setEditing(null)}}><FlightForm flight={editing} currency={trip?.currency} onSave={async(data:any)=>{editing?await upd('flights',{...editing,...data}):await add('flights',data);setModal(false);setEditing(null)}} /></Modal>}
    </div>
  )
}

function FlightForm({ flight, currency, onSave }: any) {
  const [f,setF] = useState({type:flight?.type||'ida',airline:flight?.airline||'',flightNumber:flight?.flight_number||'',originAirport:flight?.origin_airport||'',originCity:flight?.origin_city||'',destinationAirport:flight?.destination_airport||'',destinationCity:flight?.destination_city||'',departureDate:flight?.departure_date||'',departureTime:flight?.departure_time||'',arrivalDate:flight?.arrival_date||'',arrivalTime:flight?.arrival_time||'',hasLayover:flight?.has_layover===1||false,layoverAirport:flight?.layover_airport||'',layoverCity:flight?.layover_city||'',layoverArrivalTime:flight?.layover_arrival_time||'',layoverDepartureTime:flight?.layover_departure_time||'',layoverDuration:flight?.layover_duration||'',price:flight?.price||'',persons:flight?.persons||1,notes:flight?.notes||''})
  ...
  return (
    <div>
      <ErrMsg msg={error} />
      <FG label="Tipo de vuelo"><select className="form-input" value={f.type} onChange={e=>s('type',e.target.value)}><option value="ida">✈️ Vuelo de ida</option><option value="regreso">🔄 Vuelo de regreso</option></select></FG>
      ...
      {f.price&&f.persons>1&&<div style={{padding:'8px 14px',background:'rgba(74,127,165,0.06)',border:'1px solid rgba(74,127,165,0.15)',borderRadius:10,fontSize:13,color:'var(--text-mid)',marginBottom:14}}>💡 Por persona: <strong style={{color:'var(--navy)'}}>{fmt(parseFloat(String(f.price))/f.persons,currency)}</strong></div>}
      ...
      <div style={{margin:'16px 0',padding:'14px 16px',background:'var(--bg-cream)',borderRadius:12,border:'1px solid var(--border)'}}>
        ...
        {f.hasLayover&&(<>
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12}}><FG label="Código aeropuerto escala"><input className="form-input" value={f.layoverAirport} onChange={e=>s('layoverAirport',e.target.value.toUpperCase())} onKeyDown={hk} placeholder="MIA" maxLength={4} /></FG><FG label="Ciudad de escala"><input className="form-input" value={f.layoverCity} onChange={e=>s('layoverCity',e.target.value)} onKeyDown={hk} placeholder="Miami" /></FG></div>
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:12}}><FG label="Llegada a escala"><input className="form-input" type="time" value={f.layoverArrivalTime} onChange={e=>s('layoverArrivalTime',e.target.value)} /></FG><FG label="Salida de escala"><input className="form-input" type="time" value={f.layoverDepartureTime} onChange={e=>s('layoverDepartureTime',e.target.value)} /></FG><FG label="Tiempo espera"><input className="form-input" value={f.layoverDuration} onChange={e=>s('layoverDuration',e.target.value)} onKeyDown={hk} placeholder="2h 30min" /></FG></div>
        </>)}
      </div>
      ...
      <div style={{display:'flex',justifyContent:'flex-end',gap:10,marginTop:20}}><Btn onClick={handleSave} primary loading={saving}>{flight?'Guardar cambios':'Agregar vuelo'}</Btn></div>
    </div>
  )
}
    ...
    <div className="fade-in">
      <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:28,flexWrap:'wrap',gap:10}}>
        <div style={{display:'flex',gap:20,flexWrap:'wrap'}}>
          {[{c:itin.filter((a:any)=>a.status==='realizado').length,l:'realizadas',col:'#4a7c59'},{c:itin.filter((a:any)=>a.status==='pendiente').length,l:'pendientes',col:'#b87333'},{c:itin.filter((a:any)=>a.status==='cancelado').length,l:'canceladas',col:'#c45c5c'}]
            .map((s,i)=><div key={i} style={{display:'flex',alignItems:'baseline',gap:5}}><span style={{fontFamily:'Cormorant Garamond,serif',fontSize:30,fontWeight:300,color:s.col}}>{s.c}</span><span style={{fontSize:12,color:'var(--text-light)'}}>{s.l}</span></div>)}
        </div>
        {!isReader&&<Btn onClick={()=>{setEditing(null);setModal(true)}} primary>＋ Actividad</Btn>}
      </div>
      {days.map(day=>{
        const acts=itin.filter((a:any)=>a.day===day).sort((a:any,b:any)=>(a.time||'').localeCompare(b.time||''))
      ...
      {modal&&<Modal title={editing?'Editar actividad':'Nueva actividad'} onClose={()=>{setModal(false);setEditing(null)}}><ActivityForm act={editing} onSave={async(data:any)=>{editing?await upd('itinerary',{...editing,...data}):await add('itinerary',data);setModal(false);setEditing(null)}} /></Modal>}
    </div>
  )
}
...
function ActivityForm({ act, onSave }: any) {
  ...
  const s=(k:string,v:any)=>setF(p=>({...p,[k]:v}))
  const hk=(e:React.KeyboardEvent)=>{if(e.key==='Enter')e.preventDefault()}
  const handleSave=async()=>{ if(!f.name.trim()){setError('El nombre es requerido');return} setSaving(true);setError(''); try{await onSave(f)}catch(e){setError('Error al guardar.');setSaving(false)} }
  return (
    <div>
      <ErrMsg msg={error} />
      ...
      <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12}}>
        ...
        <FG label="Tipo"><select className="form-input" value={f.type} onChange={e=>s('type',e.target.value)}>{['actividades','alojamiento','transporte','comida','otros'].map(t=><option key={t}>{t}</option>)}</select></FG>
      </div>
      <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12}}>
      ...
      <FG label="Estado"><select className="form-input" value={f.status} onChange={e=>s('status',e.target.value)}><option value="pendiente">⏳ Pendiente</option><option value="realizado">✅ Realizado</option><option value="cancelado">❌ Cancelado</option></select></FG>
      <FG label="Notas"><textarea className="form-input" rows={3} value={f.note} onChange={e=>s('note',e.target.value)} /></FG>
      <div style={{display:'flex',justifyContent:'flex-end',gap:10,marginTop:20}}><Btn onClick={handleSave} primary loading={saving}>{act?'Guardar cambios':'Agregar'}</Btn></div>
    </div>
  )
}
        ...
        return (
          <div key={e.id} className="row-hover" style={{display:'flex',alignItems:'flex-start',gap:12,padding:'14px 18px',background:'var(--bg-card)',borderRadius:14,border:'1px solid var(--border)',marginBottom:8,flexWrap:'wrap'}}>
            ...
            <div style={{display:'flex',gap:14,alignItems:'center',flexWrap:'wrap'}}>
              <div style={{textAlign:'right'}}><div style={{fontSize:9,color:'var(--text-light)',textTransform:'uppercase',letterSpacing:'0.1em',marginBottom:2}}>Est.</div><div style={{fontFamily:'Cormorant Garamond,serif',fontSize:16,color:'var(--text-light)'}}>{fmt(e.estimated,trip?.currency)}</div></div>
              <div style={{textAlign:'right'}}><div style={{fontSize:9,color:'var(--text-light)',textTransform:'uppercase',letterSpacing:'0.1em',marginBottom:2}}>Real</div><div style={{fontFamily:'Cormorant Garamond,serif',fontSize:18,color:'var(--navy)'}}>{e.real!=null?fmt(e.real,trip?.currency):'—'}</div></div>
            ...
            {!isReader&&<div style={{display:'flex',gap:5}}><IBtn onClick={()=>{setEditing(e);setModal(true)}}>✏️</IBtn><IBtn onClick={()=>del('expenses',e.id)} color="#c45c5c">🗑</IBtn></div>}
          </div>
        )
      })}
      {modal&&<Modal title={editing?'Editar gasto':'Nuevo gasto'} onClose={()=>{setModal(false);setEditing(null)}}><ExpenseForm exp={editing} currency={trip?.currency} onSave={async(data:any)=>{editing?await upd('expenses',{...editing,...data}):await add('expenses',data);setModal(false);setEditing(null)}} /></Modal>}
    </div>
  )
}
...
function ExpenseForm({ exp, currency, onSave }: any) {
  ...
  const s=(k:string,v:any)=>setF(p=>({...p,[k]:v}))
  const hk=(e:React.KeyboardEvent)=>{if(e.key==='Enter')e.preventDefault()}
  const handleSave=async()=>{ if(!f.name.trim()){setError('La descripción es requerida');return} setSaving(true);setError(''); try{await onSave({...f,estimated:f.estimated!==''?parseFloat(f.estimated as string):null,real:f.real!==''?parseFloat(f.real as string):null})}catch(e){setError('Error al guardar.');setSaving(false)} }
  return (
    <div>
      <ErrMsg msg={error} />
...
function TabPlaces({ trip, items, add, upd, del, isReader }: any) {
  const [modal,setModal] = useState(false)
  const [editing,setEditing] = useState<any>(null)
  ...
  return (
    <div className="fade-in">
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'baseline',marginBottom:22,flexWrap:'wrap',gap:10}}>
        ...
        {!isReader&&<Btn onClick={()=>{setEditing(null);setModal(true)}} primary>＋ Lugar</Btn>}
      </div>
      {places.length===0&&<Empty icon="📍" title="Sin lugares" sub={isReader?'El organizador aún no ha agregado lugares':'Agrega los destinos que quieres visitar'} />}
      <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(270px,1fr))',gap:14}}>
        {places.map((p:any)=>(
          <div key={p.id} style={{background:'var(--bg-card)',borderRadius:16,border:'1px solid var(--border)',overflow:'hidden',boxShadow:'var(--shadow-card)',borderTop:`4px solid ${p.visited?'#4a7c59':'var(--border)'}`}}>
            {p.lat&&p.lng&&<LazyMap lat={p.lat} lng={p.lng} active={activeMap===p.id} onActivate={()=>setActiveMap(activeMap===p.id?null:p.id)} />}
            <div style={{padding:'14px 16px'}}>
              <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start'}}>
                ...
                {!isReader&&<IBtn onClick={()=>{setEditing(p);setModal(true)}}>✏️</IBtn>}
              </div>
            ...
            <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',padding:'10px 16px',borderTop:'1px solid var(--border)',background:'var(--bg-cream)'}}>
              <div style={{color:'#b87333',fontSize:14,letterSpacing:2}}>{[1,2,3,4,5].map(n=><span key={n} style={{opacity:n<=p.rating?1:0.18,cursor:isReader?'default':'pointer'}} onClick={()=>!isReader&&upd('places',{...p,rating:n})}>★</span>)}</div>
              <div style={{display:'flex',gap:7,alignItems:'center'}}>
                {!isReader&&<span onClick={()=>upd('places',{...p,visited:p.visited?0:1})} style={{fontSize:11,fontWeight:600,padding:'3px 10px',borderRadius:20,cursor:'pointer',transition:'all 0.15s',background:p.visited?'rgba(74,124,89,0.12)':'rgba(138,138,170,0.08)',color:p.visited?'#4a7c59':'var(--text-light)'}}>{p.visited?'✓ Visitado':'No visitado'}</span>}
                {isReader&&p.visited&&<span style={{fontSize:11,fontWeight:600,padding:'3px 10px',borderRadius:20,background:'rgba(74,124,89,0.12)',color:'#4a7c59'}}>✓ Visitado</span>}
                {!isReader&&<IBtn onClick={()=>del('places',p.id)} color="#c45c5c">🗑</IBtn>}
              </div>
            </div>
          </div>
        ))}
      </div>
      {modal&&<Modal title={editing?'Editar lugar':'Nuevo lugar'} onClose={()=>{setModal(false);setEditing(null)}}><PlaceForm place={editing} onSave={async(data:any)=>{editing?await upd('places',{...editing,...data}):await add('places',data);setModal(false);setEditing(null)}} /></Modal>}
    </div>
  )
}
...
function TabDocuments({ trip, items, add, upd, del, isReader }: any) {
  const [modal,setModal] = useState(false)
  const [editing,setEditing] = useState<any>(null)
...
function TabChecklist({ trip, items, add, upd, del, isReader }: any) {
  ...
  return (
    <div className="fade-in">
      <div style={{display:'flex',alignItems:'baseline',justifyContent:'space-between',marginBottom:8,flexWrap:'wrap',gap:8}}>
        <div style={{display:'flex',alignItems:'baseline',gap:7}}><span style={{fontFamily:'Cormorant Garamond,serif',fontSize:44,fontWeight:300,color:'var(--navy)',lineHeight:1}}><AN value={done} /></span><span style={{fontSize:13,color:'var(--text-light)'}}>/ {ck.length} listos</span></div>
        <span style={{fontSize:13,color:'var(--text-mid)',fontWeight:500}}>{ck.length>0?Math.round(done/ck.length*100):0}% empacado</span>
      </div>
      <div style={{background:'var(--bg-cream-dark)',borderRadius:4,height:5,overflow:'hidden',marginBottom:24}}><div style={{width:ck.length>0?`${Math.round(done/ck.length*100)}%`:'0%',height:'100%',borderRadius:4,background:'#b87333',transition:'width 1s cubic-bezier(0.16,1,0.3,1)'}} /></div>
      {cats.length===0&&(
        <div style={{textAlign:'center',padding:'40px 20px',background:'var(--bg-card)',borderRadius:16,border:'1px dashed var(--border)',marginBottom:24}}>
          ...
          <div style={{fontSize:13,color:'var(--text-light)',marginBottom:20}}>{isReader?'El organizador aún no ha agregado ítems':'Agrega categorías para organizar lo que llevas'}</div>
          {!isReader&&<div style={{display:'flex',gap:8,flexWrap:'wrap',justifyContent:'center'}}>{PRESET_CATS.map(p=><button key={p} onClick={()=>addCat(p)} style={{padding:'7px 14px',borderRadius:20,border:'1px solid var(--border)',background:'var(--bg)',cursor:'pointer',fontSize:12,fontWeight:500,color:'var(--text-mid)',fontFamily:'DM Sans,sans-serif'}} onMouseEnter={e=>{e.currentTarget.style.borderColor='#b87333';e.currentTarget.style.color='#b87333'}} onMouseLeave={e=>{e.currentTarget.style.borderColor='var(--border)';e.currentTarget.style.color='var(--text-mid)'}}>＋ {p}</button>)}</div>}
        </div>
      )}
        ...
        {cats.map(cat=>{
          ...
          return (
            <div key={cat} style={{background:'var(--bg-card)',borderRadius:14,border:'1px solid var(--border)',overflow:'hidden'}}>
              ...
              {ci.map((it:any)=>(
                <div key={it.id} className="row-hover" onClick={()=>!isReader&&upd('checklist',{...it,checked:it.checked?0:1})}
      ...
      {!isReader&&cats.length>0&&(
        <div style={{marginTop:20}}>
          ...
          <div style={{display:'flex',gap:8,flexWrap:'wrap',alignItems:'center'}}>
            {availablePresets.map(p=><button key={p} onClick={()=>addCat(p)} style={{padding:'6px 14px',borderRadius:20,border:'1px solid var(--border)',background:'transparent',cursor:'pointer',fontSize:12,fontWeight:500,color:'var(--text-mid)',fontFamily:'DM Sans,sans-serif'}} onMouseEnter={e=>{e.currentTarget.style.borderColor='#b87333';e.currentTarget.style.color='#b87333'}} onMouseLeave={e=>{e.currentTarget.style.borderColor='var(--border)';e.currentTarget.style.color='var(--text-mid)'}}>＋ {p}</button>)}
            {addingCat?(<div style={{display:'flex',gap:7,alignItems:'center'}}><input className="form-input" style={{padding:'6px 10px',fontSize:13,width:160}} value={newCatTxt} onChange={e=>setNewCatTxt(e.target.value)} onKeyDown={e=>{if(e.key==='Enter'){e.preventDefault();addCat(newCatTxt)}if(e.key==='Escape')setAddingCat(false)}} placeholder="Nueva categoría..." autoFocus /><button onClick={()=>addCat(newCatTxt)} style={{padding:'6px 12px',background:'#b87333',color:'white',border:'none',borderRadius:8,fontSize:12,fontWeight:600,cursor:'pointer',fontFamily:'DM Sans,sans-serif'}}>+</button><button onClick={()=>setAddingCat(false)} style={{padding:'6px 10px',background:'transparent',color:'var(--text-light)',border:'1px solid var(--border)',borderRadius:8,fontSize:12,cursor:'pointer'}}>✕</button></div>)
            :(<button onClick={()=>setAddingCat(true)} style={{padding:'6px 14px',borderRadius:20,border:'1.5px dashed var(--border)',background:'transparent',cursor:'pointer',fontSize:12,fontWeight:500,color:'var(--text-light)',fontFamily:'DM Sans,sans-serif'}}>＋ Otra...</button>)}
          </div>
        </div>
      )}
    </div>
...
function TabProposals({ trip, items, add, upd, del, isReader }: any) {
  const [modal,setModal]=useState(false)
  const props=items?.proposals||[]
  return (
    <div className="fade-in">
      {!isReader&&<div style={{display:'flex',justifyContent:'flex-end',marginBottom:20}}><Btn onClick={()=>setModal(true)} primary>＋ Propuesta</Btn></div>}
      {props.length===0&&<Empty icon="🗳" title="Sin propuestas" sub={isReader?'El organizador aún no ha agregado propuestas':'Agrega ideas para votar con tu grupo'} />}
      {props.map((p:any)=>(
        <div key={p.id} style={{background:'var(--bg-card)',borderRadius:14,border:'1px solid var(--border)',padding:'20px 22px',marginBottom:12}}>
          <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:12}}>
            <div><div style={{fontSize:15,fontWeight:700,color:'var(--text)'}}>{p.title}</div>{p.description&&<div style={{fontSize:13,color:'var(--text-mid)',marginTop:3,lineHeight:1.5}}>{p.description}</div>}</div>
            {!isReader&&<IBtn onClick={()=>del('proposals',p.id)} color="#c45c5c">🗑</IBtn>}
          </div>
      ...
      {modal&&<Modal title="Nueva propuesta" onClose={()=>setModal(false)}><ProposalForm onSave={async(data:any)=>{await add('proposals',data);setModal(false)}} /></Modal>}
    </div>
  )
}
...
function TabJournal({ trip, items, add, upd, del, isReader }: any) {
  ...
  return (
    <div className="fade-in">
      {!isReader&&<div style={{display:'flex',justifyContent:'flex-end',marginBottom:24}}><Btn onClick={()=>{setEditing(null);setModal(true)}} primary>＋ Entrada del diario</Btn></div>}
      {journal.length===0&&<Empty icon="📝" title="Tu diario está vacío" sub={isReader?'El organizador aún no ha escrito entradas':'Escribe sobre tus experiencias'} />}
      <div style={{position:'relative',paddingLeft:28}}>
        ...
        {journal.map((e:any)=>(
          <div key={e.id} style={{position:'relative',marginBottom:20}}>
            ...
            <div style={{background:'var(--bg-card)',borderRadius:16,padding:'20px 22px',border:'1px solid var(--border)',boxShadow:'var(--shadow-card)',transition:'box-shadow 0.2s'}} onMouseEnter={el=>el.currentTarget.style.boxShadow='var(--shadow-hover)'} onMouseLeave={el=>el.currentTarget.style.boxShadow='var(--shadow-card)'}>
              <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',marginBottom:10}}>
                <div><div style={{fontSize:10,fontWeight:700,letterSpacing:'0.18em',textTransform:'uppercase',color:'var(--text-light)',marginBottom:4}}>Día {e.day}{e.date?` · ${e.date}`:''}</div><div style={{fontFamily:'Cormorant Garamond,serif',fontSize:20,color:'var(--navy)'}}>{e.title}</div></div>
                {!isReader&&<div style={{display:'flex',gap:5}}><IBtn onClick={()=>{setEditing(e);setModal(true)}}>✏️</IBtn><IBtn onClick={()=>del('journal',e.id)} color="#c45c5c">🗑</IBtn></div>}
              </div>
      ...
      {modal&&<Modal title={editing?'Editar entrada':'Nueva entrada del diario'} onClose={()=>{setModal(false);setEditing(null)}}><JournalForm entry={editing} onSave={async(data:any)=>{editing?await upd('journal',{...editing,...data}):await add('journal',data);setModal(false);setEditing(null)}} /></Modal>}
    </div>
  )
}
...
function JournalForm({ entry, onSave }: any) {
  ...
  return (
    <div>
      <ErrMsg msg={error} />
      <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12}}><FG label="Día"><input className="form-input" type="number" min="1" value={f.day} onChange={e=>s('day',parseInt(e.target.value)||1)} onKeyDown={hk} /></FG><FG label="Fecha"><input className="form-input" type="date" value={f.date} onChange={e=>s('date',e.target.value)} /></FG></div>
      ...
      <FG label="¿Cómo fue el día?"><div style={{display:'flex',gap:8}}>{[1,2,3,4,5].map(n=><span key={n} style={{fontSize:28,cursor:'pointer',color:n<=f.rating?'#b87333':'var(--border)',transition:'all 0.15s'}} onClick={()=>s('rating',n)}>★</span>)}</div></FG>
      <div style={{display:'flex',justifyContent:'flex-end',gap:10,marginTop:20}}><Btn onClick={handleSave} primary loading={saving}>{entry?'Guardar':'Escribir entrada'}</Btn></div>
    </div>
  )
}
    ...
    <div className="fade-in">
        ...
        <div style={{background:'var(--bg-card)',borderRadius:16,padding:'20px 24px',border:'1px solid var(--border)',marginBottom:20}}>
          ...
          {[flIda,flReg].filter(Boolean).map((f:any)=>(
            <div key={f.id} style={{display:'flex',alignItems:'center',gap:10,padding:'12px 16px',background:'var(--bg-cream)',borderRadius:12,marginBottom:8,flexWrap:'wrap'}}>
              ...
              <div style={{flex:1,minWidth:140}}><div style={{fontFamily:'Cormorant Garamond,serif',fontSize:16,color:'var(--navy)'}}>{f.origin_airport} → {f.has_layover?`${f.layover_airport} → `:''}{f.destination_airport}</div><div style={{fontSize:12,color:'var(--text-mid)',marginTop:1}}>{f.origin_city} → {f.has_layover?`${f.layover_city} → `:''}{f.destination_city}</div></div>
              <div style={{fontSize:12,color:'var(--text-mid)',textAlign:'right'}}><div style={{fontWeight:600,color:'var(--text)'}}>{f.departure_date} · {f.departure_time}</div><div style={{marginTop:1}}>Llega {f.arrival_date} · {f.arrival_time}</div>{f.airline&&<div style={{marginTop:1,color:'var(--text-light)'}}>{f.airline} {f.flight_number}</div>}{f.price&&<div style={{marginTop:1,color:'#b87333',fontWeight:600}}>{fmt(f.price,trip?.currency)}{f.persons>1?` · ${fmt(f.price/f.persons,trip?.currency)}/persona`:''}</div>}</div>
            </div>
          ))}
        </div>
      )}
      <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:14,marginBottom:20}}>
        {[{icon:'💰',title:'Gasto total',value:fmt(real,trip?.currency),detail:`${diff>0?'+':''}${fmt(diff)} vs estimado`,color:diff>0?'#c45c5c':'#4a7c59'},{icon:'📅',title:'Actividades',value:`${done.length}/${items?.itinerary?.length||0}`,detail:`${cancelled.length} canceladas`,color:'var(--text-mid)'},{icon:'📍',title:'Lugares visitados',value:`${vis.length}/${items?.places?.length||0}`,detail:'',color:'var(--text-mid)'},{icon:'📝',title:'Entradas en el diario',value:`${items?.journal?.length||0}`,detail:'días escritos',color:'var(--text-mid)'}]
          .map((it,i)=>(
            <div key={i} style={{background:'var(--bg-card)',borderRadius:16,padding:'22px 24px',border:'1px solid var(--border)',boxShadow:'var(--shadow-card)'}}>
      ...
      {insights.length>0&&(
        <div style={{background:'var(--bg-card)',borderRadius:16,padding:'22px 26px',border:'1px solid var(--border)'}}>
          ...
          {insights.map((ins,i)=><div key={i} style={{display:'flex',gap:12,padding:'11px 0',borderBottom:i<insights.length-1?'1px solid var(--border)':'none',alignItems:'flex-start'}}><span style={{color:'#b87333',fontSize:14,marginTop:2,flexShrink:0}}>→</span><span style={{fontSize:14,color:'var(--text-mid)',lineHeight:1.6}}>{ins}</span></div>)}
        </div>
      )}
    </div>
  )
